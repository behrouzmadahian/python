...
"""
Implementation of DeepMind's asynchronous 1-step Double Dueling DQN:
Do away with experience Replay.
We need to have two networks-Double DQN:
1. Global Shared network:
        Used to apply the gradients accumulated in workers
2. Global shared target network:
        Used in calculating target values by Dueling and updated less frequently than Global Shared Network
...
if terminal state is reached then  Q(s_t+1, a_t+1) = 0

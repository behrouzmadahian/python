...
class RQnetwork(object):
    ...
    def __init__(self, weights, biases, wNames, bNames, myScope):
        ...
        # reason: we need advantage and values for each of teh frames in all traces.
        self.rnn = tf.reshape(self.rnn_h, shape = [-1, h_size])

        # split the output of conv4  into separate advantage and value streams
        self.streamAC, self.streamVC = tf.split(self.rnn, num_or_size_splits = 2, axis = 1)
...
class Experience_buffer(object):
    ...
    def sample (self, batch_size, trace_length ):
        # need to make sure experience are stored correctly sequential for each trace in training part!
...
def updateTargetGraph(tfvars, tau):
    ...
    for ix, var in enumerate(tfvars[0: mid_point]):
        ...
        update = var.value() * tau + (1 - tau) * tfvars[ix  + mid_point].value()
...
def updateTarget(op_holder,sess):
    for op in op_holder:
        sess.run(op)
    # checking if the update worked or Not!
...
# write the first line of master log file for control center
with open('./8-rdqn/Center/log.csv', 'w') as myfile :
...
with tf.Session() as sess:
    ...
    updateTarget(targetOps, sess) # set target network to be equal to primary network
    ...
    for i in range(num_episodes):
        ...
        rAll = 0
        j = 0
        state = (np.zeros([1, h_size]), np.zeros([1, h_size])) # reset RNN state
        ...
        while j < max_epLength:
            ...
            if np.random.rand(1) < e or total_steps < pre_train_steps:
                state1 = sess.run(mainQN.rnn_c, feed_dict = {mainQN.scalarInput : [s / 255.],
                                                             mainQN.trainLength : 1, mainQN.state_in :state,
                                                             mainQN.batch_size : 1})
                a = np.random.randint(0, 4)
            else:
                a, state1 = sess.run([mainQN.predict, mainQN.rnn_c], feed_dict = {mainQN.scalarInput:[s / 255.],
                                                                                 mainQN.trainLength :1,
                                                                                 mainQN.state_in :state,
                                                                                 mainQN.batch_size :1})
                a = a[0]

            s1P, r, done = env.step(a)
            s1 = processState(s1P)
            ...
            if total_steps > pre_train_steps:
                ...
                if total_steps % update_freq == 0:
                   ...
                   # print('Shape of train Batch=',trainBatch.shape)
                    ...
                    #print(trace_length, batch_size, targetQ.shape)
        ...
        print('Shape of experience for current episode=', bufferArray.shape)

        #episode_buffer = list(zip(bufferArray))
        episode_buffer = bufferArray
        ...
        #     print(total_steps, np.mean(rList[- summary_length:]), e)

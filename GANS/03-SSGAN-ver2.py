...
Assume we have very few labeled data ans a ton of unlabeled real data!
# assume half of the batches are unlabeled 
...
def discriminator(x, isTrain, reuse =False):
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        '''
        GAP = tf.reduce_mean(lrelu3, axis = [1,2])
...
d_real, d_real_logits, GAN_logits_real = discriminator(x, isTrain)
d_fake, d_fake_logits, GAN_logits_fake = discriminator(G_z, isTrain, reuse =True)
##
...
and for the Generatorrather <fake>, 
that is, we still need to model the binary classification problem for a regular GAN model.
'''
def build_loss(d_real, d_real_logits,GAN_logits_real, d_fake,
               d_fake_logits, GAN_logits_fake, label, label_mask, smooth = 0.9):
    ...
    label_mask = tf.to_float(label_mask)
    ...
    g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=GAN_logits_fake,
                                                                            labels= tf.ones_like(GAN_logits_fake)))
...
d_loss, g_loss, accuracy_all_labeled,accuracy_correct_masked  =build_loss(d_real, d_real_logits,GAN_logits_real, d_fake,
               d_fake_logits, GAN_logits_fake, label, label_mask, smooth = 0.9)
...
# results save folder
# results save folder
root = 'C:/behrouz/Research_and_Development/GANS/codes/SSGAN/results_ver2/'
...
with tf.Session() as sess:
    ...
    l_mask = np.zeros(batch_size)
    l_mask[:int(batch_size // 2)] = 1
    for epoch in range(training_epoch):
        z_ = sess.run(tf.random_uniform([batch_size, 1, 1, 100], minval=-1, maxval=1, dtype=tf.float32))

        train_acc, train_d_l, train_g_l = sess.run([accuracy_all_labeled,d_loss, g_loss], feed_dict={x:train_set,label:trainY,
                                                       z:z_, label_mask: np.ones(len(trainY)),isTrain:False})
        test_acc, test_d_l, test_g_l = sess.run([accuracy_all_labeled,d_loss,g_loss], feed_dict={x:test_set,label:testY,
                                                           z:z_, label_mask: np.ones(len(testY)), isTrain:False})
        print('Epoch= ', epoch, 'Train Accuracy=', train_acc, 'Train d loss= ', train_d_l, 'Train g loss= ', train_g_l)
        print('Epoch= ', epoch, 'Test Accuracy=', test_acc, 'Test d loss= ', test_d_l, 'Test g loss= ', test_g_l)
        ...
        for iter in range(train_set.shape[0]//batch_size):
            ...
            loss_d_, _ = sess.run([d_loss, d_optimizer], feed_dict={x:x_,label:y_, z:z_,label_mask:l_mask, isTrain:True})

...
'''
def getTrainBatch(batch_size, ids, seq_lengths):
    labels = []
    batch_seq_len = []
    arr = np.zeros([batch_size, maxSeqLength])
    for i in range(batch_size):
        if i % 2 == 0:
            num = random.randint(1, 11499) # does not include the upper limit!
            labels.append([1, 0])
        else:
            num = random.randint(13499, 24999)
            labels.append([0, 1])
        arr[i] = ids[num - 1: num]
        batch_seq_len.append(seq_lengths[num - 1])
    return arr, labels, batch_seq_len

def getTestBatch(batch_size, ids, seq_lengths):
    labels = []
    batch_seq_len = []
    arr = np.zeros([batch_size, maxSeqLength])
    for i in range(batch_size):
...
#    nextBatch, nextBatchLabels, batch_seq_len = getTrainBatch(batch_size)
#    sess.run(optimizer, {x: nextBatch, y: nextBatchLabels, seq_len:batch_seq_len})
#
#    #Write summary to Tensorboard
#    if (i % 50 == 0):
#        summary = sess.run(merged, {x: nextBatch, y: nextBatchLabels, seq_len: batch_seq_len})

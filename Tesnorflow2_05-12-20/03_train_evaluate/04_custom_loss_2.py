...
class WeightedBinaryCrossEntropy(keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        # add dimension of size 1 for broadcasting in next line!
        ce = tf.keras.losses.binary_crossentropy(y_true, y_pred, from_logits=self.from_logits)[:, None]
        print(ce.shape, '===============')
        print('Shape of labels in batch: {}'.format(y_true.shape))
...
print('Shape of labels data: {}'.format(y_train.shape))
# one hot encode

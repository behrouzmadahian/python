...
and a time series input of shape (None, 10) (that's (timesteps, features)). Our model will have two outputs
...
model.compile(
    ...
    loss=[keras.losses.CategoricalCrossentropy(from_logits=True), None])
...
a loss function in compile:
- you can pass lists of Numpy arrays
...
model.fit(data_dict_x, data_dict_y, epochs=50, batch_size=64,
          validation_data=(data_dict_x_v, data_dict_y_v))
...
data_dict_x = {'img_input': img_data[:90], 'ts_input': ts_data[:90]}
data_dict_y = {'class_output': class_targets[:90]}

data_dict_x_v = {'img_input': img_data[90:], 'ts_input': ts_data[90:]}
data_dict_y_v = {'class_output': class_targets[90:]}
# here as a show case, I am not monitoring any metrics on score_output
# and as such no need to feed the true value of score
metrics = {'class_output': keras.metrics.CategoricalAccuracy()}
"""
If you want to check metrics on score output but not added to loss:
1. make sure data_dict_y contains score_output
2. :
metrics = {'score_output': [keras.metrics.MeanAbsolutePercentageError(),
                            keras.metrics.MeanAbsoluteError()],
           'class_output': keras.metrics.CategoricalAccuracy()
...
"""
if checking metrics on score_output:
train_dataset = tf.data.Dataset.from_tensor_slices(({'img_input': img_data, 'ts_input': ts_data},

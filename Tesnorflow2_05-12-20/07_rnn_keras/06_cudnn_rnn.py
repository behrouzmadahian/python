...
batch_size = 64
# each MNISt digit image batch is a tensor of shape:  (batch_size, 28, 28)
...
def build_model(allow_cuddn_kernel=True):
  # CuDNN is only available at the layer level, and not at the cell level.
  # This means 'LSTM(units)' will use the CuDNN kernel,
  # while RNN(LSTMCell(units)) will run on non-CuDNN kernel.
  if allow_cuddn_kernel:
...
print('Shape of train x: {}; trainy: {}'.format(x_train.shape, y_train.shape))
...
slow_model = build_model(allow_cuddn_kernel=False)
...
with tf.device('CPU:0'):
    cpu_model = build_model(allow_cuddn_kernel=False)

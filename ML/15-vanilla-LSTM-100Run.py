...
results_path = 'C:/behrouz/Projects/DailyModels_new/NeuralNet/tf-5Market-model-RNN/vanilla-LSTM'
...
for i in range(0, len(markets), 1):
    ...
    curr_market_data = \
        dataProcessing.time_series_toMatrix_MAs(data, 20070418, longMA = longMA, shortMA= shortMA, lookback = 10, look_ahead = 1)
...
for R in range(nRandom_start):
    ...
    optimizer, output, sharpe_plus_l2_loss = vanilla_LSTM(x, y, weights, biases, keep_prob, curr_optimizer,
                                                                 learning_rate, objective, batch_size,
                                                                 markets=markets, activation=tf.nn.tanh,
                                                                 l2Reg=l2Reg, l2RegOutput=l2Reg * 0.5,
                                                                 l2Reg_biases=l2Reg * 0.5,
                                                                 lookback=30, hidden_size= hidden1_size,
                                                                 n_layers = 2)
    ...
    with tf.Session() as sess:
        ...
        print(' Pre- training Train basket sharpe = ', train_basket_sharpe)
        print(' Pre- training Test basket sharpe = ', test_basket_sharpe)
        ...
        for epoch in range(training_epochs):
            ...
            print(' EPOCH %d- Train basket sharpe = '%epoch , train_basket_sharpe)
            ...
            if (epoch + 1) % display_steps == 0:
                ...
                print(' EPOCH %d- Test basket sharpe = '%epoch, test_basket_sharpe)

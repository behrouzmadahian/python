...
import random
'''
A brief example of using tensorflows input pipeline to load your own custom data structures into tensorflows 
...
The input pipeline deals with reading csv files, decode file formats, restructure the data, shuffle the data,
 data augmentation or other preprocessing, and load the data in batches using threads. However, 
 we do have to write some code to get this to work.
'''
# load the label data
data_path =  'C:/behrouz/Research_and_Development/tensorflowLearning/mnist/'
...
# Do some optional preprocessing on our string lists:
...
#If you decide to use the train and test data set of mnist as it is given you
#  would just do the next steps for both sets simultaneously.

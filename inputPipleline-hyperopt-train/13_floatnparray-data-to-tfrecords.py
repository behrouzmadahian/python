...
data_files_dict['TRAIN_FILE_LIST'] = ['train_scaled_'+'%d.csv' % i for i in range(1, 11)]
data_files_dict['VALIDATION_FILE_LIST'] = ['validation_scaled_'+'%d.csv' % i for i in range(1, 11)]
data_files_dict['TEST_FILE_LIST'] = ['test_scaled_'+'%d.csv' % i for i in range(1, 11)]
N_FEATURES = 1500
...
def _floats_feature(value):
    # value here is an array -> remove the brackets around value!
...
def convert_to(data_set, name):
    ...
    for index in range(num_examples):
        ...
        x_raw = x[index, :].tolist()
        example = tf.train.Example(features=tf.train.Features(feature={
            'label': _int64_feature(int(y[index])),
            'x_raw': _floats_feature(x_raw)
        }
        ))

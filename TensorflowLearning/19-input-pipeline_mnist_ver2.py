...
def read_and_decode(filename_queue):
    ...
    As part of the API, a TFRecordReader always acts on a queue of filenames.
    It will pop a filename off the queue and use that filename until the tfRecord is empty.
    At this point it will grab the next filename off the filename queue.
    '''
    ...
    # image_resized = tf.image.resize_images(image, [28, 28])
...
def inputs(mode, batch_size, num_epochs, shuffle):
    ...
    * labels is an int32 tensor with shape [batch_size] with the true label,
    ...
    with tf.name_scope('input'):
        # shuffle makes sense if we have multiple tfRecord files!
        # creates queue of filenames! here only 1 file!!
        # reads the content all the way !
...
our input pipeline will just cycle over the training data as often as it has to -> if epochs not specified!
...
def run_training():
    ...
    with tf.Graph().as_default():
        ...
        with tf.device('/cpu:0'):
            train_images, train_labels = inputs('train', FLAGS.BATCH_SIZE, FLAGS.num_epochs, True)
            validation_images, validaton_labels = inputs('validation', FLAGS.valid_size, 1, False)
            test_images, test_labels = inputs('test', FLAGS.test_size, 1, False)
        ...
        with tf.Session(config=config) as sess:
            ...
            try:
                ...
                while not coord.should_stop():
                    ...
                    if step % 100 == 0:
                        print('Step %d: loss = %.2f, accuracy=%.2f (%.3f sec)' % (step,
                                                                                  batch_loss,
                                                                                  batch_accu,
                                                                                  duration))
                    step += 1
